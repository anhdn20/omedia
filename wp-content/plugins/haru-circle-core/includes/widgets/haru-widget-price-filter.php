 ...
 */
 class Haru_WC_Widget_Price_Filter extends Haru_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'haru-circle' );
		$this->widget_id          = 'haru_woocommerce_price_filter';
		$this->widget_name        = esc_html__( 'WooCommerce Price Filter List', 'haru-circle' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by price', 'haru-circle' ),
			...
			'price_range_size' => array(
				...
				'max'   => '',
				'std'   => 50,
				'label' => esc_html__( 'Price range size', 'haru-circle' )
			),
			'max_price_ranges' => array(
				...
				'max'   => '',
				'std'   => 10,
				'label' => esc_html__( 'Max price ranges', 'haru-circle' )
			),
			'hide_empty_ranges' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__( 'Hide empty price ranges', 'haru-circle' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( strlen( $min_price ) > 0 ) {
			$output .= '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'haru-circle' ) . '</a></li>';
		} else {
            $output .= '<li class="current">' . esc_html__( 'All', 'haru-circle' ) . '</li>';
		}
