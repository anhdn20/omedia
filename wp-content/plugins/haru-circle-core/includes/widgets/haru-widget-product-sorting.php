...
class Haru_WC_Widget_Product_Sorting extends Haru_Widget {
	...
	public function __construct() {
		...
		$this->widget_description	= esc_html__( 'Display a product sorting list.', 'haru-circle' );
		$this->widget_id          	= 'haru_woocommerce_widget_product_sorting';
		$this->widget_name        	= esc_html__( 'Haru WooCommerce Product Sorting', 'haru-circle' );
		$this->settings           	= array(
			'title'  => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Sort By', 'haru-circle' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( 1 != $wp_query->found_posts || woocommerce_products_will_display() ) {
			...
			$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
				'menu_order'	=> esc_html__( 'Default', 'haru-circle' ),
				'popularity' 	=> esc_html__( 'Popularity', 'haru-circle' ),
				'rating'     	=> esc_html__( 'Average rating', 'haru-circle' ),
				'date'       	=> esc_html__( 'Newness', 'haru-circle' ),
				'price'      	=> esc_html__( 'Price: Low to High', 'haru-circle' ),
				'price-desc'	=> esc_html__( 'Price: High to Low', 'haru-circle' )
			) );
