	...
	public function form( $instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			break;
                ...
                case "multi-select" :
                    ?>
                    <p class="widget-multi-select">
                        <label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html($setting['label']); ?></label>
                        <input name="<?php echo esc_attr($this->get_field_name( $key )); ?>" type="hidden" value="<?php echo esc_attr( $value ); ?>" />
                        <select multiple class="widefat widget-select2" style="width: 100%!important" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" data-value="<?php echo esc_attr($value) ?>">
                            <?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
				...
				case "icon":
					...
					<div style="margin: 13px 0">
						...
						<div>
							...
							<input style="float: right"  title="<?php echo esc_html__('Click to browse icon','haru-circle') ?>" class="browse-icon button-secondary" type="button" value="<?php echo esc_html__('Browse...','haru-circle') ?>" />
				...
				case "image":
					...
					<div style="margin: 13px 0">
						...
						<div class="widget-image-field">
							...
							<button style="float: right" title="<?php echo esc_html__('Click to browse image','haru-circle') ?>" class="browse-images button-secondary" type="button"><?php echo esc_html__('Browse...','haru-circle') ?></button>
						</div>
						<script type="text/javascript">
							//<![CDATA[
							jQuery(document).ready(function() {
								haru_media_init("#<?php echo esc_attr( $this->get_field_id( $key ) ); ?>",'.browse-images');
							});
							//]]>
						</script>
					</div>
