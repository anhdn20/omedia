  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        ...
        $this->SelfPath = plugins_url( 'tax-meta-class', WP_PLUGIN_DIR.'/haru-circle-core/includes/libraries/tax-meta-class' );
      }else{
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.esc_html__('Remove','haru-circle').'" title="'.esc_html__('Remove','haru-circle').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.esc_html__('Add','haru-circle').'" title="'.esc_html__('Add','haru-circle').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  if (!isset($value['id']))
	...
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".esc_html__('Remove Image', 'haru-circle')."' type='button'/>";
	else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".esc_html__('Upload Image', 'haru-circle')."' type='button'/>";
	$this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
    ...
    //  echo "<a href='#' class='at-color-select button' rel='{$field['id']}'>" . esc_html__( 'Select a color' ) . "</a>";
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__( 'Select a color','haru-circle' ) . "'/>";
