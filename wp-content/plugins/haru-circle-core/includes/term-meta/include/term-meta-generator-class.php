...
if( ! class_exists('Haru_Term_Meta') ):
	...
	final class Haru_Term_Meta {
		...
		public function load_scripts_styles() {
			...
			wp_enqueue_style( 'term-meta-style', PLUGIN_HARU_CIRCLE_CORE_URL . 'includes/term-meta/css/term-meta.css' );
			wp_enqueue_script( 'term-meta-script', PLUGIN_HARU_CIRCLE_CORE_URL . 'includes/term-meta/js/term-meta.js' );
		}
				...
				switch ( $field[ 'type' ] ) {
					...
					case 'revslider':
					   ...
					   ?>
						...
						<div class="error notice is-dismissible below-h2"><p><?php echo esc_html__('Revolution Slider is not Installed or Activated.', 'haru-circle') ?></p><button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php echo esc_html__('Dismiss this notice.', 'haru-circle') ?></span></button></div>
						<?php
						}
					...
					case 'icon':
					   ...
					   ?>
						<select name="<?php echo $field[ 'id' ] ?>" id="<?php echo $field[ 'id' ] ?>" class="<?php echo $css_class  ?>" <?php echo $field[ 'multiple' ] ?>>
					    <option value=""><?php __('-- Choose Icon --', 'haru-circle') ?></option>
						<?php
						foreach ( $field[ 'options' ] as $key => $option ) {
							echo '<option' . selected( $field[ 'value' ], $key, FALSE ) . ' value="' . $key . '">' . $option . '</option>';
						}
					...
					case 'image':
						...
						<div class="meta-image-field-wrapper">
							...
							<div style="line-height: 60px;">
								...
								<button type="button"
								        class="haru_upload_image_button button button-primary button-small"><?php echo esc_html__( 'Upload / Add image', 'haru-circle' ); ?></button>
								<button type="button" style="<?php echo (empty($field[ 'value' ])?'display:none':'') ?>"
								        class="haru_remove_image_button button button-danger button-small"><?php echo esc_html__( 'Remove image', 'haru-circle' ); ?></button>
							</div>
		...
		public function haru_placeholder_img_src() {
			return PLUGIN_HARU_CIRCLE_CORE_URL . 'includes/term-meta/images/placeholder.png';
		}
