            ...
            if ( !empty( $this->field['wbc_demo_imports'] ) ) {
                ...
                foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
                    ...
                    if ( !array_key_exists( 'imported', $imports ) ) {
                        ...
                        $imported = false;
                        $import_message = esc_html__( 'Import Demo', 'haru-circle' );
                    }else {
                        $imported = true;
                        $extra_class = 'active imported';
                        $import_message = esc_html__( 'Demo Imported', 'haru-circle' );
                    }
                    ...
                    if ( false == $imported ) {
                        echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', 'haru-circle' ).'</span><span class="button-primary importer-button import-demo-data">' . __( 'Import Demo', 'haru-circle' ) . '</span></div>';
                    }else {
                        echo '<div class="wbc-importer-buttons button-secondary importer-button">'.esc_html__( 'Imported', 'haru-circle' ).'</div>';
                        echo '<span class="spinner">'.esc_html__( 'Please Wait...', 'haru-circle' ).'</span>';
                        echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', 'haru-circle' ).'</div>';
                    }
                    echo '</div>';
                    echo '</div>';
                ...
                echo "<h5>".esc_html__( 'No Demo Data Provided', 'haru-circle' )."</h5>";
            }
