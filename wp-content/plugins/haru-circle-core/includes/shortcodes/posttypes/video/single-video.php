...
if ( ! class_exists('Haru_Framework_Shortcode_Single_Video') ) {
    class Haru_Framework_Shortcode_Single_Video {
        function __construct() {
            add_shortcode('haru_single_video', array($this, 'haru_single_video_shortcode' ));
            add_action( 'vc_before_init', array($this, 'haru_single_video_vc_map') );
        }

        function haru_single_video_shortcode($atts) {
            $atts        = vc_map_get_attributes( 'haru_single_video', $atts );
        ...
        <div class="<?php echo esc_attr( $haru_animation . ' ' . $styles_animation ); ?>">
            <?php echo haru_get_template('posttypes/video/single/'. $layout_type . '.php', array('atts' => $atts), '', '' ); ?>
        </div>
        ...
        function haru_single_video_vc_map() {
            vc_map(
                array(
                    'name'        =>  esc_html__( 'Haru Single Video', 'haru-circle' ),
                    'base'        =>  'haru_single_video',
                    'category'    =>  HARU_CIRCLE_CORE_SHORTCODE_CATEGORY,
                    'icon'        =>  'fa fa-play-circle-o haru-vc-icon',
                    'description' =>  esc_html__( 'Display single Video work', 'haru-circle' ),
                    'params'      =>  array(
                        array(
                            ...
                            'value'      => array(
                                esc_html__( 'Style 1 (Video Background)', 'haru-circle' )        => 'style_1',
                        ...
                        array(
                            'param_name'  => 'id',
                            'type'        => 'haru_video_list',
                            'admin_label' => true,
                            'heading'     => esc_html__( 'Select video', 'haru-circle' ),
                            'description' => esc_html__( 'Choose single video by search video name.', 'haru-circle'),
                        ),
                        array(
                            'type'        => 'attach_image',
    ...
    new Haru_Framework_Shortcode_Single_Video();
}