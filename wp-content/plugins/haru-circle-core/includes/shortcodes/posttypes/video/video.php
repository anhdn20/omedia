...
if ( ! class_exists('Haru_Framework_Shortcode_Video') ) {
    class Haru_Framework_Shortcode_Video {
        ...
        function haru_video_shortcode($atts) {
            ...
            $data_source = $category = $video_ids = $layout_type = $columns = $video_style = $filter = $filter_style = $filter_align = $posts_per_page = $paging_style = $orderby = $order = $el_class = $haru_animation = $css_animation = $duration = $delay = $styles_animation = '';
        ...
        <div class="<?php echo esc_attr( $haru_animation . ' ' . $styles_animation ); ?>">
            <?php echo haru_get_template('posttypes/video/'. $layout_type . '.php', array('atts' => $atts), '', '' ); ?>
        </div>  
        ...
        function haru_video_vc_map() {
            vc_map(
                array(
                    'base'        => 'haru_video',
                    'name'        => esc_html__( 'Haru Videos', 'haru-circle' ),
                    ...
                    'params'      => array(
                        array(
                            ...
                            'value'       => array(
                                ...
                                esc_html__( 'From Video IDs', 'haru-circle' ) => 'list_id'
                            )
                        ),
                        array(
                            'param_name'  => 'category',
                            'type'        => 'haru_video_categories',
                            'heading'     => esc_html__( 'Video Category', 'haru-circle' ),
                            'admin_label' => true,
                            'dependency'  => array(
                                'element' => 'data_source', 
                                'value'   => array('')
