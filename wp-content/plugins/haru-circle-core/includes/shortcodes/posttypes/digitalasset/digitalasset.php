...
if ( ! class_exists('Haru_Framework_Shortcode_Digitalasset') ) {
    class Haru_Framework_Shortcode_Digitalasset {
        function __construct() {
            add_shortcode('haru_digitalasset', array($this, 'haru_digitalasset_shortcode' ));
            add_action( 'vc_before_init', array($this, 'haru_digitalasset_vc_map') );
            $this->haru_includes();
        }
        ...
        function haru_digitalasset_shortcode($atts) {
            $atts        = vc_map_get_attributes( 'haru_digitalasset', $atts );
            $data_source = $category = $digitalasset_ids = $layout_type = $columns = $digitalasset_style = $filter = $filter_style = $filter_align = $posts_per_page = $paging_style = $orderby = $order = $el_class = $haru_animation = $css_animation = $duration = $delay = $styles_animation = '';
        ...
        <div class="<?php echo esc_attr( $haru_animation . ' ' . $styles_animation ); ?>">
            <?php echo haru_get_template('posttypes/digitalasset/'. $layout_type . '.php', array('atts' => $atts), '', '' ); ?>
        </div>  
        ...
        function haru_digitalasset_vc_map() {
            vc_map(
                array(
                    'base'        => 'haru_digitalasset',
                    'name'        => esc_html__( 'Haru Digital Asset', 'haru-circle' ),
                    'icon'        => 'fa fa-play-circle-o haru-vc-icon',
                    'description' => esc_html__( 'Display our digital asset work', 'haru-circle' ),
                    ...
                    'params'      => array(
                        array(
                            ...
                            'value'       => array(
                                ...
                                esc_html__( 'From Digital Asset IDs', 'haru-circle' ) => 'list_id'
                            )
                        ),
                        array(
                            'param_name'  => 'category',
                            'type'        => 'haru_digitalasset_categories',
                            'heading'     => esc_html__( 'Digital Asset Category', 'haru-circle' ),
                            'admin_label' => true,
                            'dependency'  => array(
                                'element' => 'data_source', 
                                'value'   => array('')
                        ...
                        array(
                            'param_name'       => 'digitalasset_style',
                            'type'             => 'dropdown',
                            'heading'          => esc_html__( 'Digital Asset Style', 'haru-circle' ),
                            'edit_field_class' => 'vc_col-sm-6 vc_column vc_column-with-padding',
                            'admin_label'      => true,
                            'value'            => array(
                        ...
                        array(
                            ...
                            'dependency' => array(
                                'element' => 'layout_type', 
                                'value'   => array('grid','masonry', 'grid_special')
                            )
                        ),
                        array(
                        ...
                        array( 
                            ...
                            'type'        => 'textfield', 
                            'admin_label' => true,
                            'description' => esc_html__( 'Number of digital assets to show', 'haru-circle' ),
                            'edit_field_class' => 'vc_col-sm-6 vc_column vc_column-with-padding',
                        ),
                        array(
    ...
    new Haru_Framework_Shortcode_Digitalasset();
}