...
if ( ! class_exists('Haru_Framework_Shortcode_Actor') ) {
    class Haru_Framework_Shortcode_Actor {
        function __construct() {
            add_shortcode('haru_actor', array($this, 'haru_actor_shortcode' ));
            add_action( 'vc_before_init', array($this, 'haru_actor_vc_map') );
            $this->haru_includes();
        }
        ...
        function haru_actor_shortcode($atts) {
            $atts        = vc_map_get_attributes( 'haru_actor', $atts );
            $data_source = $category = $actor_ids = $layout_type = $columns = $actor_style = $filter = $filter_style = $filter_align = $posts_per_page = $paging_style = $orderby = $order = $el_class = $haru_animation = $css_animation = $duration = $delay = $styles_animation = '';
        ...
        <div class="<?php echo esc_attr( $haru_animation . ' ' . $styles_animation ); ?>">
            <?php echo haru_get_template('posttypes/actor/'. $layout_type . '.php', array('atts' => $atts), '', '' ); ?>
        </div>  
        ...
        function haru_actor_vc_map() {
            vc_map(
                array(
                    'base'        => 'haru_actor',
                    'name'        => esc_html__( 'Haru Actors', 'haru-circle' ),
                    'icon'        => 'fa fa-user-o haru-vc-icon',
                    'description' => esc_html__( 'Display our actors work', 'haru-circle' ),
                    ...
                    'params'      => array(
                        array(
                            ...
                            'value'       => array(
                                ...
                                esc_html__( 'From Actor IDs', 'haru-circle' ) => 'list_id'
                            )
                        ),
                        array(
                            'param_name'  => 'category',
                            'type'        => 'haru_actor_categories',
                            'heading'     => esc_html__( 'Actor Category', 'haru-circle' ),
                            'admin_label' => true,
                            'dependency'  => array(
                                'element' => 'data_source', 
                                'value'   => array('')
                        ...
                        array(
                            ...
                            'type'       => 'haru_actor_list',
                            'heading'    => esc_html__( 'Select Actor', 'haru-circle' ),
                            'dependency' => array(
                                'element' => 'data_source', 
                                'value'   => array('list_id')
                            )
                        ...
                        array(
                            'param_name'       => 'actor_style',
                            'type'             => 'dropdown',
                            'heading'          => esc_html__( 'Actor Style', 'haru-circle' ),
                            'edit_field_class' => 'vc_col-sm-6 vc_column vc_column-with-padding',
                            'admin_label'      => true,
                            'value'            => array(
                        ...
                        array( 
                            ...
                            'type'        => 'textfield', 
                            'admin_label' => true,
                            'description' => esc_html__( 'Number of actors to show', 'haru-circle' ),
                            'edit_field_class' => 'vc_col-sm-6 vc_column vc_column-with-padding',
                        ),
                        array(
    ...
    new Haru_Framework_Shortcode_Actor();
}