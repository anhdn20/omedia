...
if ( ! class_exists('Haru_Framework_Shortcode_Film_Top_Celebrity') ) {
    class Haru_Framework_Shortcode_Film_Top_Celebrity {
        function __construct() {
            add_shortcode('haru_film_top_celebrity', array($this, 'haru_film_top_celebrity_shortcode' ));
            add_action( 'vc_before_init', array($this, 'haru_film_top_celebrity_vc_map') );
            $this->haru_includes();
        }
        ...
        function haru_film_top_celebrity_shortcode($atts) {
            $atts        = vc_map_get_attributes( 'haru_film_top_celebrity', $atts );
            $title = $layout_type = $posts_per_page = $el_class = $haru_animation = $css_animation = $duration = $delay = $styles_animation = '';
            extract(shortcode_atts(array(
                'title'          => '',
                'layout_type'    => 'top-celebrity',
                'posts_per_page' => '6',
                'el_class'       => '',
                'css_animation'  => '',
                'duration'       => '',
        ...
        function haru_film_top_celebrity_vc_map() {
            vc_map(
                array(
                    'base'        => 'haru_film_top_celebrity',
                    'name'        => esc_html__( 'Haru Film Top Celebrity', 'haru-circle' ),
                    'icon'        => 'fa fa-user-secret haru-vc-icon',
                    'description' => esc_html__( 'Display our Film Celebrity top view', 'haru-circle' ),
                    ...
                    'params'      => array(
                        ...
                        array(
                            ...
                            'admin_label'      => true,
                            'value'            => array(
                                esc_html__( 'Top List', 'haru-circle' ) => 'top-celebrity',
                            ),
                        ),
                        array( 
                            ...
                            'type'        => 'textfield', 
                            'admin_label' => true,
                            'description' => esc_html__( 'Number of film celebrities to show', 'haru-circle' ),
                            'edit_field_class' => 'vc_col-sm-6 vc_column vc_column-with-padding',
                        ),
    ...
    new Haru_Framework_Shortcode_Film_Top_Celebrity();
}