...
if ( ! class_exists('Haru_Framework_Shortcode_Director') ) {
    class Haru_Framework_Shortcode_Director {
        function __construct() {
            add_shortcode('haru_director', array($this, 'haru_director_shortcode' ));
            add_action( 'vc_before_init', array($this, 'haru_director_vc_map') );
            $this->haru_includes();
        }
        ...
        function haru_director_shortcode($atts) {
            $atts        = vc_map_get_attributes( 'haru_director', $atts );
            $data_source = $category = $director_ids = $layout_type = $columns = $director_style = $filter = $filter_style = $filter_align = $posts_per_page = $paging_style = $orderby = $order = $el_class = $haru_animation = $css_animation = $duration = $delay = $styles_animation = '';
        ...
        <div class="<?php echo esc_attr( $haru_animation . ' ' . $styles_animation ); ?>">
            <?php echo haru_get_template('posttypes/director/'. $layout_type . '.php', array('atts' => $atts), '', '' ); ?>
        </div>  
        <?php
            wp_reset_query();
        ...
        function haru_director_vc_map() {
            vc_map(
                array(
                    'base'        => 'haru_director',
                    'name'        => esc_html__( 'Haru Directors', 'haru-circle' ),
                    'icon'        => 'fa fa-user-secret haru-vc-icon',
                    'description' => esc_html__( 'Display our directors', 'haru-circle' ),
                    ...
                    'params'      => array(
                        array(
                            ...
                            'value'       => array(
                                ...
                                esc_html__( 'From Director IDs', 'haru-circle' ) => 'list_id'
                            )
                        ),
                        array(
                            'param_name'  => 'category',
                            'type'        => 'haru_director_categories',
                            'heading'     => esc_html__( 'Director Category', 'haru-circle' ),
                            'admin_label' => true,
                            'dependency'  => array(
                                'element' => 'data_source', 
                                'value'   => array('')
                        ...
                        array(
                            'param_name' => 'director_ids',
                            'type'       => 'haru_director_list',
                            'heading'    => esc_html__( 'Select Director', 'haru-circle' ),
                            'dependency' => array(
                                'element' => 'data_source', 
                                'value'   => array('list_id')
                            )
                        ...
                        array(
                            'param_name'       => 'director_style',
                            'type'             => 'dropdown',
                            'heading'          => esc_html__( 'Director Style', 'haru-circle' ),
                            'edit_field_class' => 'vc_col-sm-6 vc_column vc_column-with-padding',
                            'admin_label'      => true,
                            'value'            => array(
                        ...
                        array( 
                            ...
                            'type'        => 'textfield', 
                            'admin_label' => true,
                            'description' => esc_html__( 'Number of directors to show', 'haru-circle' ),
                            'edit_field_class' => 'vc_col-sm-6 vc_column vc_column-with-padding',
                        ),
                        array(
    ...
    new Haru_Framework_Shortcode_Director();
}