...
if ( $filters_count ) {
    ...
    $results_bar_buttons['filters'] = array(
        ...
        'title' => sprintf( __( 'Filters active %s(%s)%s', 'haru-circle' ), '<span>', $filters_count, '</span>' )
    );
}
...
if ( ! empty( $_REQUEST['s'] ) ) { // Is search query set and not empty?
    ...
    $results_bar_buttons['search_taxonomy'] = array(
        ...
        'title' => sprintf( __( 'Search results for %s&ldquo;%s&rdquo;%s', 'haru-circle' ), '<span>', esc_html( $_REQUEST['s'] ), '</span>' )
    );
}
...
// Taxonomy
    ...
    if ( is_product_category() ) {
        ...
        $results_bar_buttons['search_taxonomy']['title'] = sprintf( __( 'Showing %s&ldquo;%s&rdquo;%s', 'haru-circle' ), '<span>', esc_html( $current_term->name ), '</span>' );
    } else {
        $results_bar_class .= ' is-tag';
        $results_bar_buttons['search_taxonomy']['title'] = sprintf( __( 'Products tagged %s&ldquo;%s&rdquo;%s', 'haru-circle' ), '<span>', esc_html( $current_term->name ), '</span>' );
    }
}
...
<div class="haru-shop-results-bar <?php echo esc_attr( $results_bar_class ); ?>">
    <?php 
        ...
        foreach ( $results_bar_buttons as $button ) {
            printf( '<a href="%s" id="%s" data-shop-url="%s"><i class="ion-ios-close"></i>%s</a>',
