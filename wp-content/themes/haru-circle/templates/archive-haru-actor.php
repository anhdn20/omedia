...
if ( !in_array($archive_layout, array('full','container')) ) {
    $archive_layout = haru_get_option('archive_actor_layout');
}
// Set Default
...
if( !in_array($archive_sidebar, array('none','left','right')) ) {
    $archive_sidebar = haru_get_option('archive_actor_sidebar');
}
// Set Default
...
$archive_left_sidebar  = haru_get_option('archive_actor_left_sidebar');
$archive_right_sidebar = haru_get_option('archive_actor_right_sidebar');
// Set Default
...
$archive_display_type = haru_get_option('archive_actor_display_type');
// Set Default
...
if ( !in_array($archive_display_columns, array('2','3','4')) ) {
    $archive_display_columns = haru_get_option('archive_actor_display_columns');
}
...
if ( !in_array($archive_paging_style, array('default','load-more','infinity-scroll')) ) {
    $archive_paging_style = haru_get_option('archive_actor_paging_style');
}
// Set Default
                            ...
                            <?php
                                if ( have_posts() ) :
                                    ...
                                    while ( have_posts() ) : the_post();
                                         ...
                                         */
                                        get_template_part( 'templates/archive/content' , 'haru-actor' );
                                    endwhile;
                                else :
