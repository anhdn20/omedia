...
$enable_header_customize = get_post_meta( get_the_ID(), 'haru_' . 'enable_header_customize_left', false ); // true/false
...
if ( $enable_header_customize == '1' ) {
    $page_header_customize = get_post_meta( get_the_ID(), 'haru_' . 'header_customize_left', true );
    ...
    if ( haru_get_option('option-header-customize-left') == '1' ) {
        $enable_header_customize = true;
        $header_customize_left   = haru_get_option('header_customize_left');
        if ( isset($header_customize_left) && isset($header_customize_left['enabled']) && is_array($header_customize_left['enabled']) ) {
            foreach ( $header_customize_left['enabled'] as $key => $value ) {
        ...
        <?php foreach ( $header_customize as $key ) {
            switch ( $key ) {
                ...
                case 'search-box':
                    ...
                    break;
                case 'social-profile':
                    get_template_part('templates/header/header-customize/social-profile', 'left');
                    break;
                case 'custom-text':
                    get_template_part('templates/header/header-customize/custom-text', 'left');
                    break;
                case 'canvas-menu':
